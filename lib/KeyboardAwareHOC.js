...
function KeyboardAwareHOC(
  ...
  return class
    ...
    componentDidMount() {
      ...
      if (Platform.OS === 'ios') {
        this.keyboardWillShowEvent = Keyboard.addListener(
          'keyboardWillShow',
          (event) => { console.log('keyboardWillShow'); this._updateKeyboardSpace(event);}
        )
        this.keyboardWillHideEvent = Keyboard.addListener(
          'keyboardWillHide',
          (event) => { console.log('keyboardWillHide');  this._resetKeyboardSpace(event);}
        )
    ...
    scrollToFocusedInput = (
      ...
      console.log(keyboardOpeningTime);

      setTimeout(() => {
        if (!this.mountedComponent || this.state.isResetting) {
          return
        }
        console.log('scrollToFocusedInput');
        const responder = this.getScrollResponder()
        responder &&
    ...
    _resetKeyboardSpace = () => {
      ...
      ? _KAM_DEFAULT_TAB_BAR_HEIGHT
      : 0
      console.log('resetKeyboardSpace', keyboardSpace);
